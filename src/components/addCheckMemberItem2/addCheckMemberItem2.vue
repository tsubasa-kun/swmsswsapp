...
<script type="text/javascript">
    ...
    export default {
        ...
        data() {
            return {
                score: this.member.JCFS,
                describe: this.member.BZ,
                imgs: [],
                actions: [{name: '拍照', method: this.takePhoto},
                    {name: '从相册中选择', method: this.selectPhoto}],
                sheetVisible: false,
                imgTokens: this.member.FJ
            }
        },
        created() {
            if (this.member.FJ) {
                this.imgs = [];
                this.imgs.push(Utils.methods.getFileByToken(this.member.FJ));
            }
        },
        methods: {
            ...
            getData: function () {
                let data = {
                    "XSJCBM": this.member.JCBM,
                    "XSBH": this.member.XSBH || '',
                    "JCFS": this.score || '',
                    "XSFJ": this.imgTokens || '',
                    "XSBZ": this.describe || ''
                };
        ...
        watch: {
            ...
            member: function () {
                this.score = this.member.JCFS;
                this.describe = this.member.BZ;
                this.imgTokens = this.member.FJ;
                this.imgs = [];
                if (this.member.FJ) {
                    this.imgs.push(Utils.methods.getFileByToken(this.member.FJ));
                }
